...
ad_party <-
  fetch::fetchGoogle(
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vS6fE8z9WFZPDqpSQCVA3AA9AI5vjYf1LF6VXYCn6LiW3-AjqFojVcravtGbCwbUplaue48Dle-IAug/pub?gid=1509796885&single=true&output=csv"
  )
ad_party <- ad_party %>%
...
#user interface
ui <- fluidPage(
  titlePanel(paste0("Political Advertising on Google Platforms - ",
                    startDate,
                    " to ",
                    endDate
  )),
  sidebarLayout(
    sidebarPanel(
      paste0("Last data download: ", downDate),
      checkboxGroupInput(
        inputId = "partyInput",
      ...
      helpText(
        "Advertiser political leaning was assessed based on 2-person independent 
        web search. Cases where agreement wasn't reached were labeled 'Undetermined'."
      ),
      sliderInput(inputId = "daysInput",
                  label = "Number of days aired",
                  value = c(1, max_day),
                  min = 1,
                  max = max_day,
                  step = 5
     ),
     checkboxGroupInput(inputId = "ad_typeInput",
                        label = "Ad Type",
    ...
    mainPanel(plotOutput("coolplot"), #output placeholder 1
              br(), br(),
