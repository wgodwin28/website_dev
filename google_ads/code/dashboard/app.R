...
sidebar <- dashboardSidebar(
  sidebarMenu(
    ...
    menuItem("Time series", tabName = "tseries", icon = icon("bar-chart-o")),
    menuItem("Ad Table", tabName = "ad_table", icon = icon("table")),
  ...
  tabItems(
    ...
    tabItem(tabName = "tseries",
      fluidRow(
        ...
        box(width = 8, height = 14,
            plotOutput("coolplot")
        )
      ),
      fluidRow(
        box(
          helpText("Click over to 'Ad Table' tab to see individual contributors for each scenario")
        )
      )
    ),
    #third tab page
    tabItem(tabName = "ad_table",
      #box(
...
ui <- dashboardPage(
  dashboardHeader(title = "Google Political Ads"),
  sidebar,
  body
)
