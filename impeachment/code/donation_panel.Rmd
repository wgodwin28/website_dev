---
title: "Effects of Impeachment on Donald Trump's Donors"
author: "Will Godwin"
date: "1/17/2020"
output:
  epuRate::epurate:
...
## Results
During this time there were XX total contributions, totaling $XXXX. As shown below, the majority of these donations came in during July-September months. Average size of donation, a proxy for strength of support, _________ over the course of 2016. Weekly donation totals were fairly steady from January-June, with zip codes that Trump won with the popular vote consistently outspending zip codes that Clinton won. Donation campaigns in June and July kicked off a significant surge in incoming money to Trump's PACs Impeachment inquiry began in ____, as we see an uptick in donation support.
</div>
```{r, echo=F, message=F, warning=F}
#read in files
...
```{r facet wrap on state, echo=F, message=F, warning=F}
...
```{r facet wrap on state for number unique voters, echo=F, message=F, warning=F, fig.cap="The gray vertical lines are defined as follows: 1st-Trump's birthday, 2nd-Whistleblower complaint becomes public, 3rd-Pelosi announce formal impeachment inquiry."}
#facet wrap the plot on specific states
states_ab <- c("WI", "MI", "PA", "AZ", "NC", "FL")
plotStatesFacet <- function(states, df_plot=df1, df_zip_fun=df_zip){
  #find zip codes for that state
