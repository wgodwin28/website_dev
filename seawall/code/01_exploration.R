...
shape_data <- left_join(shape_data, df_pop, by=c("district", "state")) %>%
  mutate(pop=as.numeric(as.character(pop)),
         ...
         value_percap_cat=cut(value_percap, breaks = c(0,350,1500,4000,10000,40000), labels = c("0-350", "350-1,500", "1,500-4,000", "4,000-10,000", "10,000+"))) %>%
...
#plot for US
pdf(paste0(out_dir, "us_cost_2040_wsurge_percap.pdf"))
shape_data %>%
  mutate(value=value/10^6) %>%
...
#plot by distance for US
pdf(paste0(out_dir, "us_cost_2040_wsurge_permile.pdf"))
shape_data %>%
